...
bool Writer::encrypt_by_key(const uint8_t* data, size_t data_length, const TOX_PASS_KEY* key, TOX_ERR_ENCRYPTION* error) const
{
...
bool Writer::encrypt_by_passphrase(const uint8_t* data, size_t data_length, uint8_t* passphrase, size_t pplength, TOX_ERR_ENCRYPTION* error) const
{
    Mem_Guard pass_guard(passphrase, pplength);
    TOX_PASS_KEY key;
    TOX_ERR_KEY_DERIVATION _error;

    if ( !key.derive_from_pass_with_random_salt(pass_guard.ptr, pplength, &_error) ) {
    ...
    return this->encrypt_by_key(data, data_length, &key, error);
}
